...
# =============================================================================
# Minimal cohort loading functions
# =============================================================================
class minimal_participant:
    '''Participant class object.
    Attributes:
    - id: String. Participant's id.
...
class minimal_trajectory:
    ...
    def __init__(self, part_id=None, mutation=None,  data=None,
                 variant_class=None, gradient=None):
...
def minimal_load(df, participant_id=None, gene=None, key=None, age=None, vaf=None, AO=None, DP=None,
                 cohort_name=None, variant_class=None, export_name=False, create_dict=False):
    """ Transform the full dataset into a list of participant class objects.
    Parameters:
    - df: pandas DataFrame. pandas dataframe in ARCHER_DX format.
    Returns:
    ...
    if key is None:
        exit += 1
        warnings.warn("key parameter not recognized", Warning)

    if participant_id is None:
        exit +=1
        warnings.warn("participant_id parameter not recognized", Warning)

    if age is None:
        exit +=1
        warnings.warn("age parameter not recognized", Warning)
    ...
    for part_id in tqdm(id_list):
        part_df = df[df[participant_id] == part_id]
        # create a list of participant objects with id ad data attributes.
        new_part = minimal_participant(id=part_id,
                               cohort=cohort_name,
                               data=part_df)

        # Create trajectories for each key in the participant
        for key_id in new_part.data[key].unique():
            minimal_load_key(new_part, key=key, age=age, key_id=key_id, vaf=vaf, AO=AO, DP=DP, variant_class=variant_class)

        if len(new_part.trajectories) > 0:
            # find commmon data points to all trajectories in new_part
            age_sets = [set(traj.data.age) for traj in new_part.trajectories]
            common_ages = set.intersection(*age_sets)
            for traj in new_part.trajectories:
                traj.data = traj.data.loc[traj.data['age'].isin(list(common_ages))]
    ...
    if create_dict is True:
        # Create dictionary assigning colors to each gene present in the cohort
        gene_color_dict.create_dict(df[gene].unique())
        variant_color_dict.create_dict(df[variant_class].unique())
        

    if export_name is not False:
        # Export cohort.
        auxiliary.dill_export(cohort_name, path='../exports/',
                              file_name=export_name)

    return cohort


def minimal_load_key(part, key, age, key_id, vaf, AO, DP, variant_class):
    """Creates a trajectory class object associated to a slice of the DataFrame
    associated with a genetic mutation, or key. During this process, the
    participant's trajectory list and mutation_list is updated.
    Parameters:
    ...
    """
    # Create a copy of the data slice corresponding to a trajectory
    key_data = part.data.loc[part.data[key] == key_id].copy()
    key_data = key_data.reset_index()
    ...
    # append key to mutation_list
    part.mutation_list.append(key_id)

    # create a new trajectory
    key_trajectory = minimal_trajectory(
        part_id=part.id,
        mutation=key_id, data=key_data[[age, 'delta_t', vaf, AO, DP]])
    # Rename columns
    key_trajectory.data = key_trajectory.data.rename(columns={age:'age', vaf:'AF', AO:'AO', DP:'DP'})
    # Transform AO and DP to int
    key_trajectory.data[['AO', 'DP']] = key_trajectory.data[['AO', 'DP']].astype(int)

    # Set Variant classification attribute
    key_trajectory.variant_class = key_data[variant_class].iloc[0]

    # compute overall gradient and update total_grad
    key_trajectory.gradient = (np.diff(key_data[vaf].iloc[[0, -1]])
                               / np.sqrt(np.diff(key_data[age].iloc[[0, -1]])))[0]

    key_trajectory.filter = True
    part.trajectories.append(key_trajectory)