...
read_depth = 1000
true_s = 0.1
true_x0_het = 1   # 1 heterozygous cell at t=0
true_x0_hom = 1   # 1 homozygous cell at t=0
LOH_time = 20
sample_times = np.array([60, 70, 80])

# Express truth in inference model terms (h, x0)
# x0 is clone size at first observation (t=30), not at t=0
true_x0 = (true_x0_het + true_x0_hom) * np.exp(true_s * sample_times[0])  # ≈ 3616
true_h   = true_x0_het / (true_x0_het + true_x0_hom)                       # = 0.5

# Relative times: first observation = t=0, subsequent = time since first obs
relative_times = sample_times - sample_times[0]  # [0, 10, 20]

# ======================
# 2. Clone growth models
# ======================
def het_clone(t, s, x0_het):
    return x0_het * np.exp(s * t)

def hom_clone(t, s, x0_hom):
    return x0_hom * np.exp(s * t)

# ======================
# 3. Generate synthetic data
# ======================
x_het = het_clone(sample_times, true_s, true_x0_het)
x_hom = hom_clone(sample_times, true_s, true_x0_hom)
true_vaf = (x_het + 2 * x_hom) / (2 * (N_w + x_het + x_hom))

rng = np.random.default_rng(42)
AO_obs = rng.binomial(read_depth, true_vaf)

print("=== Data Generation ===")
print(f"True parameters: s={true_s}, x0_het={true_x0_het}, x0_hom={true_x0_hom}")
print(f"Equivalent inference params: x0={true_x0:.1f} (at t={sample_times[0]}), h={true_h}")
...
# ======================
# 4. PyMC inference model — UNCHANGED except s_times_loh
# ======================
with pm.Model() as model:
    # Three parameters to infer
    s = pm.Uniform("s", 0, 1, initval=0.5)
    h = pm.Uniform("h", 0, 1, initval=0.5)
    x0 = pm.Uniform("x0", 1, 1e8, initval=1000)  # Clone size at first observation

    # Cap exponentials to prevent overflow
    MAX_EXP = 20
    # Use relative times: first observation = 0, so x0 is size at t=30
    s_times = pm.math.minimum(s * relative_times, MAX_EXP)
    # Both subclones grow at same rate, no LOH gating
    s_times_loh = s_times
...
az.plot_posterior(
    trace,
    var_names=["s", "h", "x0"],
    ref_val=[true_s, true_h, true_x0],  # now 0.25, 0.5, 2
    ax=axes
)
plt.tight_layout()
...
param_names = ['s', 'h', 'x0']
true_values = {'s': true_s, 'h': true_h, 'x0': true_x0}  # 0.25, 0.5, 2
...
axes_pair[0, 0].axvline(true_s,  color="red", ls="--", lw=2, label=f"True s={true_s}")
axes_pair[0, 0].legend()
axes_pair[1, 1].axvline(true_h,  color="red", ls="--", lw=2, label=f"True h={true_h}")
axes_pair[1, 1].legend()
axes_pair[2, 2].axvline(true_x0, color="red", ls="--", lw=2, label=f"True x0={true_x0}")
...
def full_vaf(t, s, h, x0):
    MAX_EXP = 20
    # t here is absolute time; convert to relative to first observation
    rel_t = t - sample_times[0]
    s_times = np.minimum(s * rel_t, MAX_EXP)
    # Both subclones grow from first observation
    x_het = x0 * np.exp(s_times)
    x_hom = x0 * np.exp(s_times)
    het = h * x_het
    hom = (1 - h) * x_hom
    return (het + 2 * hom) / (2 * (N_w + het + hom))

t_grid = np.linspace(0, 100, 300)
