...
# set random seed
np.random.seed(123)
with open('../exports/LBC/LBC_syn_cohort_1.pk', 'rb') as f:
    participant_syn = pk.load(f)

with open('../exports/LBC/LBC_non_syn_cohort_1.pk', 'rb') as f:
...
mutations = set([mut.key for mut in mut_obj_list_ns 
                 if mut.mut_prob/mut.artefact_prob>1])

len(mutations.intersection(data.Mutation_ID))

len(data.Mutation_ID)
len(mutations)
